...
#                              songsDB.Explicit, songsDB.Liveness, songsDB.Loudness, songsDB.Popularity, songsDB.Speechiness, songsDB.Tempo).filter(songsDB.Artist == Artist).all()
#     session.close()
...
#Route for data by individual Artist
@app.route("/by_artist_song/<Artist>")
def by_artist_song(Artist):
    session = Session(engine)
