...
d3.csv(
  ...
  function (data) {
    ...
    // Add X axis
    var x = d3.scaleLinear().domain([-10, 110]).range([0, width]);
    svg
      .append("g")
    ...
    var kde = kernelDensityEstimator(kernelEpanechnikov(7), x.ticks(20)); // increase this 40 for more accurate density.
    var allDensity = [];
    for (i = 0; i < n; i++) {
      key = categories[i];
      density = kde(
